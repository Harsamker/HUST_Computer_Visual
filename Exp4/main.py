...
def apply_cam(image_path, model, target_layer, target_class):
    ...
    # Layer-CAM
    grayscale_cam_layer = layer_cam(input_tensor=input_tensor, targets=[target_category])
    layer_cam_image = show_cam_on_image(rgb_img, grayscale_cam_layer[0, :], use_rgb=True)
...
num_cols = len(target_classes) * 2
fig, axes = plt.subplots(num_rows, num_cols, figsize=(12, 8))

for i, img_path in enumerate(image_paths):
    for j, target_class in enumerate(target_classes):
        grad_cam_image, layer_cam_image = apply_cam(img_path, model, target_layer, target_class)
        # 显示 Grad-CAM
